...
const copyRemoveArray = [...array];
copyRemoveArray.splice(index, 1)
return copyRemoveArray;
}
...
const copyArtist = [...array]
copyArtist.push({name, years,  genre, nationality,  bio});
  return copyArtist;
 }
 console.log('Task 6', addArtist(artists, 'Tayler', '2002-Current', 'Full Stack', 'White', 'Student at BloomTech'));
// const nameinsert = {object here}
...
function artistByCountry(array, filterT2){
  const countryArtist = [];
  for(let i = 0; i < array.length; i++){
    if(array[i].nationality === filterT2){
      countryArtist.push(array[i].name);
    }
  }
